...
class SparseSdfNetwork(nn.Module):
    ...
    def get_conditional_volume(self, feature_maps, partial_vol_origin, proj_mats, sizeH=None, sizeW=None, lod=0,
                               ...
                               ):# feature_maps.shape([1, 32, 56, 256, 256]) partial_vol_origin.shape=torch.Size([1, 3]) proj_mats.shape([1, 32, 4, 4])
        """
        ...
        minimum_visible_views = np.min([1, N_views - 1])# 1
        # import ipdb; ipdb.set_trace()
        outputs = {}
        pts_samples = []

        # ------------------------------------------------------------------------
        # step 2 : 第一个网络：cost volume

        # * use fused pyramid feature maps are very important
        if self.compress_layer is not None:# True
            feats = self.compress_layer(feature_maps[0])
        else:
            feats = feature_maps[0]
        feats = feats[:, None, :, :, :]  # [V, B, C, H, W] torch.Size([32, 1, 16, 256, 256])
        KRcam = proj_mats.permute(1, 0, 2, 3).contiguous()  # [V, B, 4, 4] torch.Size([32, 1, 4, 4])
        interval = 1

        if self.lod == 0:
            # ------------------------------------------------------------------------
            # step 2.1 : generate new coords
            coords = generate_grid(self.vol_dims, 1)[0]
            coords = coords.view(3, -1).to(device)  # [3, num_pts] torch.Size([3, 884736])
            up_coords = []
            for b in range(bs):
                up_coords.append(torch.cat([torch.ones(1, coords.shape[-1]).to(coords.device) * b, coords]))
            up_coords = torch.cat(up_coords, dim=1).permute(1, 0).contiguous()# torch.Size([778820, 4])
            # * since we only estimate the geometry of input reference image at one time;
            # * mask the outside of the camera frustum
            # import ipdb; ipdb.set_trace()
            # 理解：2D fea -> 3D volume
            # /root/Project/DHG/One-2-3-45/reconstruction/ops/back_project.py line 5
            # ------------------------------------------------------------------------
            # step 2.1 : generate new coords
            frustum_mask = back_project_sparse_type(
                up_coords, partial_vol_origin, self.voxel_size,
                feats, KRcam, sizeH=sizeH, sizeW=sizeW, only_mask=True)  # [num_pts, n_views] torch.Size([884736, 32])
            frustum_mask = torch.sum(frustum_mask, dim=-1) > minimum_visible_views  # ! here should be large torch.Size([884736])
            up_coords = up_coords[frustum_mask]  # [num_pts_valid, 4] torch.Size([778820, 4])
        ...
        # ------------------------------------------------------------------------
        # step 2.2 : get multiview_features
        # ----back project----
        ...
        multiview_features, multiview_masks = back_project_sparse_type(
            ...
            KRcam, sizeH=sizeH, sizeW=sizeW)  # (num of voxels, num_of_views, c), (num of voxels, num_of_views)
                                              ...
                                              # multiview_features.shape=([778820, 32, 16]) multiview_masks.shape([778820, 32])
        ...
        # ------------------------------------------------------------------------
        # step 2.3 : get cost volume
        # compute variance for all images features
        volume = self.aggregate_multiview_features(multiview_features, multiview_masks) # volume.Size([778820, 32]) 
        ...
        if self.lod != 0:# False
            ...
            feat = volume # torch.Size([778820, 32])
        ...
        feat = self.sparse_costreg_net(sparse_feat)# torch.Size([778820, 16])


        # ------------------------------------------------------------------------
        # step 4 : 第二个网络：geometry volume
        dense_volume, valid_mask_volume = self.sparse_to_dense_volume(up_coords[:, 1:], feat, self.vol_dims, interval,
                                                                      ...
                                                                      #dense_volume.shape([1, 16, 96, 96, 96]) valid_mask_volume.shape([1, 1, 96, 96, 96])
        ...
        outputs['coords_scale%d' % self.lod] = generate_grid(self.vol_dims, interval).to(device)# torch.Size([1, 3, 96, 96, 96])

        return outputs

    def sdf(self, pts, conditional_volume, lod):# torch.Size([262144, 3]) torch.Size([1, 16, 96, 96, 96])
        num_pts = pts.shape[0]
        device = pts.device
        pts_ = pts.clone()
        ...
        # import ipdb; ipdb.set_trace()
        sampled_feature = grid_sample_3d(conditional_volume, pts)  # [1, c, 1, 1, num_pts] torch.Size([1, 16, 1, 1, 262144])
        sampled_feature = sampled_feature.view(-1, num_pts).permute(1, 0).contiguous().to(device) # torch.Size([262144, 16])

        sdf_pts = self.sdf_layer(pts_, sampled_feature)# torch.Size([262144, 128])

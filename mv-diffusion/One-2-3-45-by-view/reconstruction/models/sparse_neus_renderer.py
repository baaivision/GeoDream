...
class SparseNeuSRenderer(nn.Module):
    ...
    def render(self, rays_o, rays_d, near, far, sdf_network, rendering_network,# torch.Size([512, 3]) rays_d=torch.Size([512, 3])
               perturb_overwrite=-1,# -1
               background_rgb=None,# 1
               alpha_inter_ratio=0.0,
               # * related to conditional feature
               lod=None,# 0
               ...
               # * 2d feature maps
               feature_maps=None,# torch.Size([33, 56, 256, 256])
               color_maps=None,# torch.Size([33, 3, 256, 256])
               w2cs=None,# torch.Size([33, 4, 4])
               intrinsics=None,# torch.Size([33, 3, 3])
               img_wh=None,
               query_c2w=None,  # -used for testing
               if_general_rendering=True,
               if_render_with_grad=True,
        ...
        # Up sample
        if self.n_importance > 0:# True
            with torch.no_grad():
                pts = rays_o[:, None, :] + rays_d[:, None, :] * z_vals[..., :, None]

                sdf_outputs = sdf_network.sdf(# sdf_network!!!!!!!!!!!!
                    pts.reshape(-1, 3), conditional_volume, lod=lod)
                # pdb.set_trace()
                sdf = sdf_outputs['sdf_pts_scale%d' % lod].reshape(N_rays, self.n_samples - bg_num)# torch.Size([512, 64])
                ...
                for i in range(n_steps):
                    ...
                    z_vals, sdf = self.cat_z_vals(
                        ...
                        conditional_valid_mask_volume=conditional_valid_mask_volume,
                    )# torch.Size([512, 128]) # torch.Size([512, 64])
        ...
        # Render
        if bg_num > 0:
            z_vals = torch.cat([z_vals, z_vals_bg], dim=1)# torch.Size([512, 128])
        ...
        color_fine = ret_fine['color']# torch.Size([512, 3])
        ...
        result = {
            'depth': depth,# torch.Size([512, 1])
            'color_fine': color_fine,# torch.Size([512, 3])
            'color_fine_mask': color_fine_mask,# torch.Size([512, 1])

...
def gen_random_rays_from_single_image(H, W, N_rays, image, intrinsic, c2w, depth=None, mask=None, dilated_mask=None,
                                      importance_sample=False, h_patch_size=3):# 256 256 512 image.shape([3, 256, 256]) depth.Size([256, 256]) mask.shape.Size([256, 256]) dilated_mask=None importance_sample=None h_patch_size=3
    """
    ...
    if dilated_mask is None:# True
        dilated_mask = mask

    if not importance_sample:# False
        ...
        try:
            ...
            print("high=p_valid.shape[0] = ", p_valid.shape[0])# high=p_valid.shape[0] = 0
            print("dilated_mask.shape: ", dilated_mask.shape)#  torch.Size([256, 256])
            print("dilated_mask valid number", dilated_mask.sum())# dilated_mask valid number tensor(0.)

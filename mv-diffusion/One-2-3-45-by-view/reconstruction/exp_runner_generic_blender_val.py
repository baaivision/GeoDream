...
class Runner:
    ...
    def export_mesh(self, resolution=360):
        ...
        batch['batch_idx'] = torch.tensor([x for x in range(self.batch_size)])# self.batch_size=6

        # - warmup params
        if self.num_lods == 1:# true
            alpha_inter_ratio_lod0 = self.get_alpha_inter_ratio(self.anneal_start_lod0, self.anneal_end_lod0)# alpha_inter_ratio_lod0=1.0 
        else:
            alpha_inter_ratio_lod0 = 1.
        alpha_inter_ratio_lod1 = self.get_alpha_inter_ratio(self.anneal_start_lod1, self.anneal_end_lod1)# alpha_inter_ratio_lod1=1.0

...
class Runner:
    def __init__(self, conf_path, mode='train', is_continue=False,
        ...
        if self.mode == 'export_mesh' or self.mode == 'val':# False
    ...
    def train(self):
        ...
        for epoch_i in range(epochs_needed):
            ...
            for batch in dataloader:
                ...
                # - warmup params
                if self.num_lods == 1:# true
                    alpha_inter_ratio_lod0 = self.get_alpha_inter_ratio(self.anneal_start_lod0, self.anneal_end_lod0)
                else:
                    alpha_inter_ratio_lod0 = 1.
                alpha_inter_ratio_lod1 = self.get_alpha_inter_ratio(self.anneal_start_lod1, self.anneal_end_lod1)# 0
                ...
                if self.iter_step % self.report_freq == 0: # self.report_freq=100
                ...
                if self.iter_step % self.save_freq == 0:# 5000保存一次
                    self.save_checkpoint()

                if self.iter_step % self.val_freq == 0:# 1600 val一次
                    self.validate()
    ...
    def save_checkpoint(self):
        ...
        save_state_dict(self.sdf_network_lod1, checkpoint, "sdf_network_lod1")# None

        save_state_dict(self.rendering_network_outside, checkpoint, 'rendering_network_outside')# None
        save_state_dict(self.rendering_network_lod0, checkpoint, "rendering_network_lod0")
        save_state_dict(self.rendering_network_lod1, checkpoint, "rendering_network_lod1")# None

        save_state_dict(self.variance_network_lod0, checkpoint, 'variance_network_lod0')
        save_state_dict(self.variance_network_lod1, checkpoint, 'variance_network_lod1')# None

        save_state_dict(self.pyramid_feature_network, checkpoint, 'pyramid_feature_network')
        save_state_dict(self.pyramid_feature_network_lod1, checkpoint, 'pyramid_feature_network_lod1')# None

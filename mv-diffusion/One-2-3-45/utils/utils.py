...
def gen_poses(shape_dir, pose_est):
    ...
    img_ids = [f"{num}.png" for num in range(40)]
    plus_azi = [30, 90, 150, 210, 270, 330]
    #plus_azi = [330, 270, 210, 150, 90, 30]
    plus_ele = [-30, 20, -30, 20, -30, 20]
    #plus_ele = [-10, 10, -10, 10, -10, 10]
    
    stage_1_ele = [90.0]*6 + [(x + 90.0) for x in plus_ele]
    elevations = ([90.0] + [(x + 90.0) for x in plus_ele])*4
    #elevations = stage_1_ele + elevations
    elevations = np.radians(stage_1_ele + elevations)
    
    stage_1_azi = [0.0]*6 + [(x + 0.0)%360 for x in plus_azi]
    azimuths = [0.0] + [(x + 0.0)%360 for x in plus_azi] + [90.0] + [(x + 90.0)%360 for x in plus_azi] + [180.0] + [(x + 180.0)%360 for x in plus_azi] + [270.0] + [(x + 270.0)%360 for x in plus_azi]
    # stage_1_azi = [0.0]*6 + [(x + 0.0)%360 for x in plus_azi]
    # azimuths = [0.0] + [(x + 0.0)%360 for x in plus_azi] + [90.0] + [(x + 90.0)%360 for x in plus_azi] + [180.0] + [(x + 180.0)%360 for x in plus_azi] + [270.0] + [(x + 270.0)%360 for x in plus_azi]
    #azimuths = stage_1_azi + azimuths
    azimuths = np.radians(stage_1_azi + azimuths)
    print(azimuths)
    print(elevations)
    #import pdb;pdb.set_trace
    
    input_poses = calc_pose(elevations, azimuths, len(azimuths)).cpu().numpy()
...
def remove_duplicates_with_same_ab(arr):
    seen = set()
    result = []
...
#     #img_ids = ["0.png"]
#     img_ids = ["0.png"] + [f"{num}.png" for num in range(13,40)]
#     print(img_ids)
#     plus_azi = [30, 90, 150, 210, 270, 330]
#     #plus_azi = [330, 270, 210, 150, 90, 30]
#     plus_ele = [-30, 20, -30, 20, -30, 20]
...
#     #stage_1_ele = [90.0]*6 + [(x + 90.0) for x in plus_ele]
#     stage_1_ele = [90.0]
#     elevations = [(x + 90.0) for x in plus_ele] + ([90.0] + [(x + 90.0) for x in plus_ele])*3
#     elevations = stage_1_ele + elevations
#     print(elevations)
...
#     #stage_1_azi = [0.0]*6 + [(x + 0.0)%360 for x in plus_azi]
#     stage_1_azi = [0.0]
#     azimuths = [(x + 0.0)%360 for x in plus_azi] + [90.0] + [(x + 90.0)%360 for x in plus_azi] + [180.0] + [(x + 180.0)%360 for x in plus_azi] + [270.0] + [(x + 270.0)%360 for x in plus_azi]
#     # stage_1_azi = [0.0]*6 + [(x + 0.0)%360 for x in plus_azi]
#     # azimuths = [0.0] + [(x + 0.0)%360 for x in plus_azi] + [90.0] + [(x + 90.0)%360 for x in plus_azi] + [180.0] + [(x + 180.0)%360 for x in plus_azi] + [270.0] + [(x + 270.0)%360 for x in plus_azi]
#     azimuths = stage_1_azi + azimuths
#     print(azimuths)
...
#     #     r_set.append((elevations[i], azimuths[i],img_ids[i]))
#     # r = remove_duplicates_with_same_ab(r_set)
#     # print(r)
#     # print(len(r))
#     # input_poses = []
    
#     # for i in range(len(azimuths)):
#     #    input_poses.append(calc_pose2(elevations[i], azimuths[i], 1).cpu().numpy())

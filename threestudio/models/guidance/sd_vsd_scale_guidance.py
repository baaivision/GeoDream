...
@threestudio.register("stable-diffusion-vsd-guidance-scale")
class StableDiffusionVSDGuidanceScale(BaseModule):
    @dataclass
    ...
    #         - self.extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape)
    #         * noise
    #     )
    ...
    def compute_grad_vsd(
        ...
        latents_recon = self.scheduler_sample.predict_start_from_noise(latents_noisy, t, noise_pred_pretrain)

        # clip or rescale x0
        if self.cfg.recon_std_rescale > 0:
            import pdb; pdb.set_trance()
            latents_recon_nocfg = self.scheduler_sample.predict_start_from_noise(latents_noisy, t, noise_pred_pretrain_text)
            ...
            factor = (latents_recon_nocfg([1,2,3],keepdim=True) + 1e-8) / (latents_recon.std([1,2,3],keepdim=True) + 1e-8)
            
            latents_recon_adjust = latents_recon.clone() * factor.squeeze(1)
        ...
        latents_recon_lora = self.scheduler_lora_sample.predict_start_from_noise(latents_noisy, t, noise_pred_est)
        if self.cfg.recon_std_rescale > 0:
            import pdb; pdb.set_trance()
            latents_recon_nocfg_lora = self.scheduler_lora_sample.predict_start_from_noise(latents_noisy, t, noise_pred_est_camera)
            #latents_recon_nocfg_reshape = latents_recon_nocfg.view(-1,self.cfg.n_view, *latents_recon_nocfg.shape[1:])
            #latents_recon_reshape = latents_recon.view(-1,self.cfg.n_view, *latents_recon.shape[1:])
            factor = (latents_recon_nocfg_lora([1,2,3],keepdim=True) + 1e-8) / (latents_recon_lora.std([1,2,3],keepdim=True) + 1e-8)
            
            latents_recon_adjust_lora = latents_recon_lora.clone() * factor.squeeze(1)
            latents_recon_lora = self.cfg.recon_std_rescale * latents_recon_adjust_lora + (1-self.cfg.recon_std_rescale) * latents_recon_lora

        loss = 0.5 * F.mse_loss(latents_recon_lora, latents_recon.detach(), reduction="sum") / latents_recon_lora.shape[0]
        grad = torch.autograd.grad(loss, latents_recon_lora, retain_graph=True)[0]
        return grad, loss
    ...
    def forward(
        ...
        **kwargs,
        ...
        grad, loss_vsd = self.compute_grad_vsd(
